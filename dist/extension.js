...
const DeepSeekViewProvider_1 = require("./views/DeepSeekViewProvider");
const applyPatch_1 = require("./workspace/applyPatch");
const DeepSeekPlaywright_1 = require("./deepseek/DeepSeekPlaywright");
...
function activate(context) {
    ...
    if (typeof vscode.window.registerWebviewViewProvider !== "function") {
        vscode.window.showErrorMessage("当前 VSCode/Cursor 版本不支持 registerWebviewViewProvider：侧边栏 Webview 视图无法加载。请升级宿主版本。");
        return;
    }
    const deepseek = new DeepSeekPlaywright_1.DeepSeekPlaywright(context.globalStorageUri);
    const provider = new DeepSeekViewProvider_1.DeepSeekViewProvider(context, deepseek);
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(DeepSeekViewProvider_1.DeepSeekViewProvider.viewType, provider, {
        // 需求：每次进入侧边栏不保留上次 UI（避免“回来还是上次对话”）
        webviewOptions: { retainContextWhenHidden: false }
    }));
    context.subscriptions.push(vscode.commands.registerCommand("deepseekCoder.applyPatchFromClipboard", async () => {
        const text = await vscode.env.clipboard.readText();
        if (!text?.trim()) {
            await provider.notify("⚠️ 剪贴板为空：没有可应用的补丁内容。");
            return;
        }
        await (0, applyPatch_1.applyPatchTextWithPreviewAndConfirm)(text);
    }));
    context.subscriptions.push(vscode.commands.registerCommand("deepseekCoder.addSelectionToContext", async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("deepseekCoder.rollbackLastChange", async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("deepseekCoder.installPlaywrightChromium", async () => {
        ...
        await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: "Deepseek Coder：正在安装 Playwright Chromium…", cancellable: false }, async () => {
            await new Promise((resolve, reject) => {
                const node = process.execPath;
                const child = (0, child_process_1.spawn)(node, [cliJs, "install", "chromium"], {
