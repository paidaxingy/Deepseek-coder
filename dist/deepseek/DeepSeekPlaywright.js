...
exports.DeepSeekPlaywright = void 0;
const vscode = __importStar(require("vscode"));
const playwright_1 = require("playwright");
class DeepSeekPlaywright {
    storageDir;
    ctx;
    page;
    ...
    isDefaultDeepThinkEnabled() {
        // 默认不启用（用户要求：默认不打开）
        return vscode.workspace.getConfiguration().get("deepseekCoder.defaultDeepThink") ?? false;
    }
    async ensureDeepThinkEnabled(page, debug, desired) {
        ...
        const detect = async () => {
            return await page
                ...
                const pickClickable = (el) => {
                    const h = el;
                    const cand = h.closest("button,[role='button'],[aria-pressed],[aria-checked],[class*='ds-toggle-button'],[class*='toggle'],[class*='Toggle'],label") || h;
                    return cand;
                };
                const getState = (clickEl) => {
                    const aria = clickEl.getAttribute("aria-pressed") ||
                    ...
                    if (/\bds-toggle-button\b/.test(cls) && !/\bds-toggle-button--selected\b/.test(cls))
                        return "off";
                    return "unknown";
                };
                const centerOf = (el) => {
                ...
                const ranked = labels
                    ...
                    return {
                        labTag: lab.tagName,
                        labClass: String(lab.getAttribute?.("class") || "").slice(0, 80),
                        clickTag: clickEl.tagName,
                        clickClass: String(clickEl.getAttribute("class") || "").slice(0, 120),
                        aria: clickEl.getAttribute("aria-pressed") ||
                            clickEl.getAttribute("aria-checked") ||
                            clickEl.getAttribute("data-state") ||
                            "",
                ...
                .catch(() => ({ found: false, state: "unknown" }));
        };
        try {
            ...
            for (let attempt = 0; attempt < 3; attempt++) {
                ...
                if (ok)
                    return;
                if (typeof cur.cx === "number" && typeof cur.cy === "number") {
                    try {
                        await page.mouse.click(cur.cx, cur.cy, { delay: 20 });
                    }
                    catch {
                        // ignore
                    }
    ...
    async openAndLetUserLogin() {
        ...
        this.ctx = await playwright_1.chromium.launchPersistentContext(userDataDir, {
            headless: false
        });
     ...
     */
    async setDeepThink(desired, debug) {
     ...
     * 检测：暂停按钮消失，发送按钮出现
     */
    async waitForDeepSeekReady(page, debug) {
     ...
     */
    async sendPromptToDeepSeek(page, prompt, debug, deepThink, messageLocator, baseCount) {
        ...
        const fillPromptSafely = async () => {
            ...
            if (actual.length < Math.min(20, prompt.length)) {
                ...
                await page
                    ...
                    const ta = tas[tas.length - 1] || document.querySelector("textarea");
                    if (!ta)
                        return;
                    ta.focus();
                    ta.value = v;
        ...
        const isMessageCountIncreased = async () => {
            if (!messageLocator)
                return false;
        ...
        const waitForSentSignal = async (ms = 3500) => {
        ...
        let sent = false;
        const clickSendNearTextareaViaJS = async () => {
            const ok = await page
                .evaluate(() => {
                const visible = (el) => {
                    const h = el;
                ...
                const scope = ta.closest("form") ||
                    ta.closest("div") ||
                    ta.parentElement;
                if (!scope)
                    return false;
                const candidates = Array.from(scope.querySelectorAll("button, div[role='button'], [class*='ds-icon-button'], [class*='_7436101']")).filter(visible);
                ...
                .catch(() => false);
            if (ok)
                debug?.({ level: "info", msg: "clicked send via JS (near textarea)" });
            return ok;
        };
        const clickSendBySelectors = async () => {
     ...
     * - 不会发送新消息（避免污染对话）
     */
    async diagnoseStopAndThink() {
        if (!this.page)
            return "Playwright 未连接。请先运行：Deepseek Coder: Playwright 打开 DeepSeek（可登录）";
        if (this.page.isClosed()) {
            this.page = undefined;
        ...
        try {
            const info = await page.evaluate(() => {
                ...
                const stopLike = clickable.filter((el) => {
                    ...
                    const hasRectIcon = !!el.querySelector?.("svg rect");
                ...
                const msg = Array.from(document.querySelectorAll(".ds-message, [class*='ds-message']")).slice(-1)[0];
    ...
    async sendWithDebug(prompt, debug, opts) {
        if (!this.page) {
    ...
    async sendStreaming(prompt, onUpdate, opts) {
        return await this.sendStreamingWithDebug(prompt, onUpdate, opts);
    }
    async sendStreamingWithDebug(prompt, onUpdate, opts) {
        if (!this.page) {
            throw new Error("Playwright 未连接。请先点击「Playwright 打开 DeepSeek（可登录）」。");
        }
        if (this.page.isClosed()) {
        ...
        // 等待新消息并获取内容
        const assistantText = await this.waitForNewMessageStreaming(page, messageLocator, baseCount, onUpdate, opts?.signal, opts?.debug);
        return { assistantText };
    }
    /**
        ...
        for (let attempt = 0; attempt < 4; attempt++) {
            ...
            if (!clicked) {
                const ok = await page
                    ...
                    const host = textarea.closest("form") ||
                        textarea.closest("div[class*='input']") ||
                        textarea.closest("div[class*='Input']") ||
                        textarea.closest("div[class*='chat']") ||
                        textarea.parentElement ||
                        document.body;
                    const candidates = Array.from(host.querySelectorAll("div[class*='_7436101'], div.ds-icon-button, div[class*='ds-icon-button'], button, [role='button']")).filter((el) => isVisible(el));
     ...
     */
    async waitForNewMessageStreaming(page, messageLocator, baseCount, onUpdate, signal, debug) {
        const start = Date.now();
        // 1. 等待新消息出现（消息数量增加）
        ...
        while (Date.now() - start2 < maxWaitMs) {
            if (signal?.aborted)
                throw new Error("已取消");
            const msgText = (await lastMessage
                .evaluate((el) => {
    ...
    buildStructuredParts(thinkingHeader, thinkingBody, answer) {
        const parts = [];
    ...
    async extractStructuredAssistantTextFromMessage(lastMessage, fallbackInnerText, debug) {
        ...
        try {
            const raw = await lastMessage
                ...
                const lines = thinkText
                    ...
                    .map((x) => x.trim())
                    .filter(Boolean);
                const header = lines.find((l) => /^Thought for\b/i.test(l) || /^Thinking\b/i.test(l) || /^思考\b/.test(l) || /^推理\b/.test(l)) || "Thinking";
                ...
                .catch(() => ({ ok: false, thinkingHeader: "", thinkingBody: "", answer: "" }));
            if (raw?.ok) {
     ...
     * 如果没有特殊格式，返回原文
     */
    extractKeyContentFromRawReply(rawReply) {
        ...
        const toolKeyIdx = Math.max(text.toLowerCase().lastIndexOf("toolcall"), text.search(/"tool"\s*:/), text.search(/"(readFile|listDir|searchText)"\s*:/));
        if (toolKeyIdx !== -1) {
    ...
    isActionableKeyContentComplete(key) {
        ...
        if (/^```toolplan\b[\s\S]*?```$/m.test(s))
            return true;
        if (/^```toolcall\b[\s\S]*?```$/m.test(s))
            return true;
        if (/^```diff\b[\s\S]*?```$/m.test(s))
            return true;
        if (/^```bash\b[\s\S]*?```$/m.test(s))
            return true;
        // bare diff
    ...
    isLikelyCompleteUnifiedDiff(key) {
    ...
    async waitForLatestAssistantText(page) {
    ...
    async getAssistantLocatorAndCount(page, debug) {
    ...
    async waitForAssistantTextFromIndex(page, assistantLocator, baseCount) {
        const start = Date.now();
        // 等待“新一条 assistant 出现”
        let gotNew = false;
        while (Date.now() - start < 30000) {
    ...
    async waitForAssistantTextFromIndexStreaming(page, assistantLocator, baseCount, onUpdate, signal, debug) {
        // 等待"新一条 assistant 出现"
        const start = Date.now();
        let gotNew = false;
     ...
     */
    extractKeyContentFromDelta(delta, debug) {
    ...
    extractDiffBlock(text) {
        ...
        const isUiNoiseLine = (t) => {
            const s = String(t || "").trim();
            if (!s)
                return false;
            if (/^DeepThink$/i.test(s))
                return true;
            if (/^AI-generated$/i.test(s))
                return true;
            if (/^Search$/i.test(s))
                return true;
            if (/^Reading$/i.test(s))
                return true;
            if (/^for reference only$/i.test(s))
                return true;
            if (/^This response is AI-generated$/i.test(s))
                return true;
            if (s === "复制" || s === "重新生成" || s === "编辑")
                return true;
            if (/^Copy$/i.test(s))
            ...
            if (/^停止生成$/i.test(s))
                return true;
            if (/^正在思考/.test(s) || /^正在生成/.test(s))
                return true;
            return false;
        };
        // markdown fence：可能是模型把 diff 包在 ```diff ... ``` 里，或消息尾部的 fence
        // 注意：真实代码也可能出现 ```（markdown 文件），因此不能无条件当作终止符
        const isFenceLine = (t) => /^```[\w-]*$/i.test(String(t || "").trim());
        const nextNonEmptyTrimmed = (fromIdx) => {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (trimmed) {
                ...
                const shouldTerminate = (!next && (isUiNoiseLine(trimmed) || isFenceLine(trimmed))) ||
            ...
            // 判断这一行是否"像 diff 内容"
            const isDiffMeta = line.startsWith("diff --git ") ||
            ...
            const isDiffContent = line.startsWith(" ") ||
                line.startsWith("+") ||
                line.startsWith("-") ||
            ...
            if (isDiffMeta || isDiffContent) {
                ...
                // 避免“到某一行突然被截断”的问题。
                const looksLikeCode = sawAnyChangeLine &&
    ...
    extractKeyContentFromMainText(text) {
        const raw = (text || "").trim();
        if (!raw)
            return "";
        const candidates = [];
    ...
    async waitForMainKeyContent(page, debug) {
        const start = Date.now();
        let last = "";
        while (Date.now() - start < 90000) {
    ...
    async waitForMainKeyContentStreaming(page, onUpdate, signal, debug) {
        const start = Date.now();
        let last = "";
        let stableCount = 0;
        while (Date.now() - start < 90000) {
    ...
    async getFallbackTextLocator(page, debug) {
    ...
    async getFallbackCombinedText(page, debug) {
        ...
        const tryRead = async (name, locator, useInnerText = false) => {
            const t0 = Date.now();
...
exports.DeepSeekPlaywright = DeepSeekPlaywright;
//# sourceMappingURL=DeepSeekPlaywright.js.map