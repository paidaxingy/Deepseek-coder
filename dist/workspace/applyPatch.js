...
exports.applyPatchTextDirectly = applyPatchTextDirectly;
exports.applyPatchTextWithPreviewAndConfirm = applyPatchTextWithPreviewAndConfirm;
...
function isProbablyUnifiedDiff(text) {
...
async function getWorkspaceFileUri(pathFromRepoRoot) {
...
async function createUntitledDoc(content, nameHint) {
...
async function writeFileUtf8(uri, content) {
...
function stripWholeFileMarkdownFencesIfLikely(opts) {
    ...
    if (allowFences.has(ext))
        return { text: opts.text, changed: false };
    const lines = String(opts.text ?? "").replace(/\r\n/g, "\n").split("\n");
    const isFence = (l) => /^```[\w-]*\s*$/.test(String(l || "").trim());
    const firstNonEmpty = lines.findIndex((l) => String(l || "").trim() !== "");
    if (firstNonEmpty === -1)
        return { text: opts.text, changed: false };
    let lastNonEmpty = lines.length - 1;
    while (lastNonEmpty >= 0 && String(lines[lastNonEmpty] || "").trim() === "")
        lastNonEmpty--;
    if (lastNonEmpty < 0)
        return { text: opts.text, changed: false };
    let changed = false;
...
function normalizeHunksForDiffLib(patchText) {
    ...
    const isValidHunkLine = (l) => l.startsWith(" ") || l.startsWith("+") || l.startsWith("-") || l.startsWith("\\");
    let i = 0;
...
function splitDiffIntoFileBlocks(patchText) {
...
function isCreateBlock(text) {
    return /(^|\n)---\s+\/dev\/null\s*($|\n)/.test(text);
}
function isDeleteBlock(text) {
    return /(^|\n)\+\+\+\s+\/dev\/null\s*($|\n)/.test(text);
}
/**
 * 直接应用补丁（不需要确认，完全自动化）
...
async function applyPatchTextDirectly(patchText) {
    ...
    for (const b of blocks) {
        ...
        try {
            newText = (0, diff_1.applyPatch)(oldTextForPatch, normalized);
        }
        catch (e) {
...
async function applyPatchTextWithPreviewAndConfirm(patchText) {
    ...
    const rb = (0, rollback_1.beginRollbackCapture)("手动确认应用补丁（diff）");
    let appliedCount = 0;
    for (const b of blocks) {
        const isCreate = isCreateBlock(b.text);
        const isDelete = isDeleteBlock(b.text);
        ...
        if (isDelete) {
            ...
            const pick = await vscode.window.showWarningMessage(`确认删除文件 ${targetPath} ?`, { modal: true }, "删除", "跳过");
            if (pick !== "删除")
                continue;
            await rb.addFile(targetPath);
            if (!exists) {
        ...
        await vscode.commands.executeCommand("vscode.diff", left, right, `Patch Preview: ${actionLabel} ${targetPath}`);
        const pick = await vscode.window.showWarningMessage(`确认${actionLabel}文件 ${targetPath} ?`, { modal: true }, "应用", "跳过");
        if (pick !== "应用")
            continue;
        await rb.addFile(targetPath);
        const stripped = stripWholeFileMarkdownFencesIfLikely({ targetPath, text: newText });
