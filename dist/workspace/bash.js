...
exports.extractBashBlock = extractBashBlock;
exports.cleanBashBlock = cleanBashBlock;
exports.splitBashCommands = splitBashCommands;
exports.assessBashRisk = assessBashRisk;
exports.checkBashCommandSafety = checkBashCommandSafety;
...
function assessBashRisk(block) {
    ...
    if (/(^|\s)sudo(\s|$)/.test(s))
        reasons.push("包含 sudo");
    if (/(^|\n)\s*rm\s+.*-rf(\s|$)/.test(s) || /(^|\n)\s*rm\s+-rf(\s|$)/.test(s))
        reasons.push("包含 rm -rf");
    if (/(^|\n)\s*dd(\s|$)/.test(s))
        reasons.push("包含 dd");
    if (/(^|\n)\s*(mkfs|fdisk|parted)\b/.test(s))
        reasons.push("包含磁盘/分区相关命令（mkfs/fdisk/parted）");
    if (/(^|\n)\s*(chmod|chown)\b/.test(s))
        reasons.push("包含权限/属主修改（chmod/chown）");
    ...
    if (/(^|[^&])&([^&]|$)/.test(s))
        reasons.push("包含后台运行（&）");
    if (/`/.test(s) || /\$\(/.test(s))
        reasons.push("包含命令替换（` 或 $()）");
    if (/(^|\n)\s*(curl|wget)\b[\s\S]*\|\s*(sh|bash)\b/.test(s))
        reasons.push("包含 curl/wget | sh/bash");
    if (/(^|\n)\s*(sh|bash)\s+-c\b/.test(s))
        reasons.push("包含 sh/bash -c");
    // 写入系统敏感目录
...
function checkBashCommandSafety(cmd, mode = "relaxed") {
    const s = (cmd || "").trim();
    if (!s)
    ...
    if (mode === "safe") {
        ...
        if (/(^|[^&])&([^&]|$)/.test(s))
            return { ok: false, reason: "包含危险 shell 语法（&）" };
        if (/`/.test(s) || /\$\(/.test(s))
            return { ok: false, reason: "包含危险 shell 语法（` 或 $()）" };
    }
