...
export class DeepSeekPlaywright {
  ...
  private async waitForNewMessageStreaming(
    ...
    while (Date.now() - start2 < maxWaitMs) {
      ...
      if (lastText && stableCount >= stableNeed) {
        const gen = await page
          .evaluate(() => {
            ...
            for (let i = 0; i < svgs.length; i++) {
              ...
              if (!parent) continue;
              const r = parent.getBoundingClientRect();
              if (r.width > 20 && r.top > window.innerHeight * 0.5) return { hasStop: true, method: "rect" };
            }
            ...
            for (let i = 0; i < spinners.length; i++) {
              ...
              const r = el.getBoundingClientRect();
              if (r.width > 0 && r.height > 0) return { hasStop: true, method: "spinner" };
            }
        ...
        if (stillGenerating) {
          debug?.({ level: "info", msg: "message stabilized but still generating; keep waiting", data: { method: (gen as any)?.method } });
        } else {
            ...
            if (codeBlocks.length > 0) {
              ...
              const processNode = (node: Node): string => {
                ...
                if (tagName === "pre" || tagName === "code" || htmlEl.className?.includes("code")) {
                  ...
                  if (lines.length > 0) {
                    return Array.from(lines).map(l => (l as HTMLElement).innerText || l.textContent || "").join("\n");
                  }
   ...
   */
  private looksLikeIncompleteDiff(key: string): boolean {
    const raw = String(key || "").trim();
    if (!raw) return false;
    ...
    if (this.isLikelyCompleteUnifiedDiff(key)) return false;
    ...
    if (/diff --git /.test(s)) {
      ...
      const hasHunk = /\n@@ /.test(s);
      // 有 diff --git 但缺少必要结构
      if (!hasMinus || !hasPlus || !hasHunk) return true;
    }
   ...
   * 用于防止在代码块生成到一半时提前结束
   */
  private hasUnclosedCodeBlock(key: string): boolean {
    const raw = String(key || "").trim();
    if (!raw) return false;
   ...
   */
  private async scrollCodeBlocksToLoadAll(
    lastMessage: ReturnType<Page["locator"]>,
    debug?: DebugFn
  ): Promise<void> {
    try {
